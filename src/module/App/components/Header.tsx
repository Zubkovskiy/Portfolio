  ...
  return (
    <>
      ...
      >
        <NavLink
          to="/"
          className="group flex items-center gap-4 text-3xl font-bold text-primary leading-none cursor-pointer"
        >
          <p className="group-hover:text-[var(--color-text)] duration-300">ZBS</p>

          <RotatingText
            texts={["Zubkovskyi", "Bohdan", "Serhiyovych"]}
            mainClassName="opacity-0 max-w-0 group-hover:opacity-100 group-hover:max-w-xs transition-all duration-500 overflow-hidden justify-center rounded-lg"
            staggerFrom="last"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "-120%" }}
